...
def animate_skeleton_3d(
    ...
    root_rel = tensor_data - tensor_data[:, 2:3]
    span = max(1.5, float(np.percentile(np.abs(root_rel), 99)))
    ...
    lines = [
        ax.plot([], [], [], c="blue", lw=2, zorder=2)[0] for _ in JOINT_CONNECTIONS
    ]
    ...
    def update(frame_idx: int):
        ...
        for ln, (a, b) in zip(lines, JOINT_CONNECTIONS):
            ln.set_data(
                np.array([frame[int(a), 0], frame[int(b), 0]]),
    ...
    if output_filename:
        os.makedirs(
            os.path.dirname(os.path.abspath(output_filename)) or ".", exist_ok=True
        )
...
def grid_static(
    ...
    for i in range(N):
        ...
        for a, b in JOINT_CONNECTIONS:
            ax.plot(
                [frame[int(a), 0], frame[int(b), 0]],
                [frame[int(a), 1], frame[int(b), 1]],
                [frame[int(a), 2], frame[int(b), 2]],
                c="blue",
                lw=1.5,
            )
        ax.set_box_aspect([1, 1, 1])
